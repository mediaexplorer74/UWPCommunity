    ...
    public static class NavigationManager
    {
        ...
        public static Tuple<Type, object> ParseProtocol(Url ptcl)
        {
            ...
            try
            {
                ...
                //TODO
                string rootPath = path.Split('/', (char)StringSplitOptions.RemoveEmptyEntries)[0];
                switch (rootPath)
                {
                    case "nointernet":
                        return new Tuple<Type, object>(typeof(Views.Subviews.NoInternetPage), 
                            /*queryParams*/default);

                    default:
                        PageInfo pageInfo = MainPage.Pages.Find(p => p.Path == rootPath);
                        destination = pageInfo != null ? pageInfo.PageType : typeof(HomeView);
                        return new Tuple<Type, object>(destination, /*queryParams*/default);
                }
            }
            catch
            {
