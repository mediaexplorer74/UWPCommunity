...
namespace Microsoft.Toolkit.Mvvm.Messaging.Messages
{
    ...
    public class AsyncCollectionRequestMessage<T> : IAsyncEnumerable<T>
    {
        ...
        public IEnumerable<T> GetAsyncEnumerator(CancellationToken cancellationToken = default)
        {
            if (cancellationToken.CanBeCanceled)
            {
                _ = cancellationToken.Register(this.cancellationTokenSource.Cancel);
            ...
            foreach (var (task, func) in this.responses)
            {
                ...
                if (task is not null)
                {
                    yield return task.Result;//.ConfigureAwait(false);
                }
               else
                {
                    yield break; //return func!(cancellationToken).ConfigureAwait(false);
                }
            }
        }
    ...
    public interface IAsyncEnumerator<out T> : IEnumerable
    {
    }
