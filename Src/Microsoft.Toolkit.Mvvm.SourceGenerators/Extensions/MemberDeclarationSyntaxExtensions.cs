...
namespace Microsoft.Toolkit.Mvvm.SourceGenerators.Extensions
{
    ...
    internal static class MemberDeclarationSyntaxExtensions
    {
        ...
        {
            int index = 0;// memberDeclaration.Modifiers.IndexOf(oldKind);
        ...
        {
            int index = 0;// memberDeclaration.Modifiers.IndexOf(kind);

            if (index != -1)
            {
                return null;// memberDeclaration.WithModifiers(memberDeclaration.Modifiers.RemoveAt(index));
            }
